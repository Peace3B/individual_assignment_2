...
class _HabitListState extends State<HabitList> {
  ...
  void _editItem(HabitItem habitItem) async {
    final editedItem = await Navigator.of(context).push<HabitItem>(
      MaterialPageRoute(
        builder:
            (context) => EditItem(habitItem: habitItem, userId: widget.userId),
      ),
    );

    if (editedItem != null) {
      setState(() {
        final index = _habitItems.indexWhere(
  ...
  Widget build(BuildContext context) {
    Widget content = const Center(child: Text('No goals have been added yet!'));
